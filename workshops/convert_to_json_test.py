...
RAW_PATH = os.path.join(BASE_DIR, 'data', 'raw', 'thai_raw.txt')
OUT_PATH = os.path.join(BASE_DIR, 'data', 'processed', 'thai_raw.json')
...
VIOLATION_KEYWORDS = [
    "ฝ่าฝืน", "กระทำผิด", "ไม่ปฏิบัติตาม", "ประมาท", "หวาดเสียว",
    "ขับรถในขณะมึนเมา", "เกินกว่าที่กฎหมายกำหนด", "ชนแล้วหนี", 
    "ไม่หยุดรถ", "ไม่ได้รับใบอนุญาต", "ค้างชำระค่าปรับ", "ฝ่าไฟแดง",
    "ขับรถย้อนศร", "ไม่สวมหมวกนิรภัย", "ไม่รัดเข็มขัดนิรภัย"
]

TRAFFIC_LAW_KEYWORDS = [
    "พ.ร.บ. จราจร", "ทางหลวง", "ทางเดินรถ", "ผู้ขับขี่", "กฎจราจร",
    "เครื่องหมายจราจร", "สัญญาณไฟจราจร", "พนักงานเจ้าหน้าที่",
    "ใบอนุญาตขับขี่", "แผ่นป้ายทะเบียน", "การตรวจวัดแอลกอฮอล์",
    "ความเร็วเกินกำหนด", "ช่องเดินรถประจำทาง", "ที่ห้ามจอด"
]

VEHICLE_KEYWORDS = [
    "รถยนต์", "รถจักรยานยนต์", "รถบรรทุก", "รถสาธารณะ",
    "รถส่วนบุคคล", "ยานพาหนะ", "เครื่องยนต์", "ล้อเลื่อน"
]

# Negation patterns (สำหรับการตรวจสอบว่า "ไม่ผิด" หรือ "ได้รับยกเว้น")
NEGATION_PATTERNS = [
    r"ไม่(?:ได้)?\s*(?:ฝ่าฝืน|กระทำผิด|ประมาท)",
    r"มิได้\s*(?:ฝ่าฝืน|กระทำผิด|ประมาท)",
    r"มีใบอนุญาตขับขี่",
    r"ถูกต้องตามระเบียบ",
    r"ไม่เข้าข่ายความผิด",
    r"ได้รับยกเว้น",
    r"เหตุสุดวิสัย",
    r"ไม่ถือว่าเป็นการฝ่าฝืน",
]
...
CONTEXT_BOOST_KEYWORDS = {
    "DUI": ["มึนเมา", "สุรา", "แอลกอฮอล์", "ทดสอบลมหายใจ", "ปริมาณแอลกอฮอล์"],
    "DRIVING_LICENSE": ["ใบอนุญาตขับขี่", "ใบขับขี่", "ไม่ได้รับอนุญาต", "ถูกพักใช้ใบอนุญาต"],
    "TRAFFIC_BEHAVIOR": ["ประมาท", "หวาดเสียว", "ขับรถย้อนศร", "ฝ่าสัญญาณไฟ", "ความเร็วเกินกำหนด"],
    "HIT_AND_RUN": ["ชนแล้วหนี", "ไม่หยุดรถให้ความช่วยเหลือ", "ไม่แสดงตัว", "หลบหนี"],
    "PENALTY": ["จำคุก", "ปรับ", "ระวางโทษ", "กักขัง", "พักใช้ใบอนุญาต", "เพิกถอนใบอนุญาต"],
}
...
LEGAL_HIERARCHY = {
    ...
    "NOTIFICATIONS": {"level": 5, "name": "ประกาศ/ระเบียบ"}, # เพิ่มเพื่อรองรับประกาศเรื่องค่าปรับจราจร
}
...
AMBIGUOUS_PATTERNS = [
    ("ใบอนุญาต", ["ใบอนุญาต", "ใบ-อนุญาต"]),
    ("แอลกอฮอล์", ["แอลกอฮอล์", "แอล-กอ-ฮอล์"]),
    ("พนักงานเจ้าหน้าที่", ["พนักงานเจ้าหน้าที่", "พนักงาน-เจ้าหน้าที่"]),
    ("รถจักรยานยนต์", ["รถจักรยานยนต์", "รถ-จักรยานยนต์"]),
    ("เครื่องหมายจราจร", ["เครื่องหมายจราจร", "เครื่องหมาย-จราจร"]),
]
...
def get_matching_rules(text: str) -> Tuple[List[str], float]:
    ...
    if has_violation:
        ...
        has_traffic = any(kw in text for kw in TRAFFIC_LAW_KEYWORDS) # แก้ชื่อตัวแปรให้ตรงกับที่ประกาศไว้ข้างบน
        has_vehicle = any(kw in text for kw in VEHICLE_KEYWORDS)     # ถ้าคุณใช้ VEHICLE_KEYWORDS แทน Copyright

    if has_traffic:
        rules_matched.append("traffic")
    if has_vehicle:
        rules_matched.append("vehicle")

    type_match = 1.0 if ( has_traffic or has_vehicle) else 0.0
    max_score = 2.0
