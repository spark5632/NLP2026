...
DATA_PATH = (
    os.path.join(BASE_DIR, 'data', 'processed', 'thai_raw.json')
    if os.path.isdir(os.path.join(BASE_DIR, 'data'))
    else os.path.join(_parent, 'data', 'processed', 'thai_raw.json')
)
...
class ThaiLegalTokenizer:
    ...
    "ขับรถย้อนศร", "ไม่สวมหมวกนิรภัย", "ไม่รัดเข็มขัดนิรภัย","พ.ร.บ. จราจร", "ทางหลวง", 
    "ทางเดินรถ", "ผู้ขับขี่", "กฎจราจร","เครื่องหมายจราจร", "สัญญาณไฟจราจร", 
    "พนักงานเจ้าหน้าที่","ใบอนุญาตขับขี่", "แผ่นป้ายทะเบียน", "การตรวจวัดแอลกอฮอล์",
